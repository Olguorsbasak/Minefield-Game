...
public class MineFieldGame implements MouseListener {
	...
	public MineFieldGame() {
		openButton = 0;
		frame = new JFrame("Game: MineField");
		...
		for (int row = 0; row < board.length; row++) {
			for (int col = 0; col < board[0].length; col++) {
				...
				frame.add(b);
				b.addMouseListener(this);//aşağıda yazdığımızı refere ediyoruz.
				board[row][col] = b;
			}
		}
		...
		for (int row = 0; row < board.length; row++) {
			for (int col = 0; col < board[0].length; col++) {
				if (board[row][col].isMine()) {
					...
					//bir sayı bir stringi birleştirince string olur o yüzden sonuna + ""
					//koyarsak string olur bu bir trick
	...
	public void open(int r, int c) {
		if (r < 0 || r >= board.length || c < 0 || c >= board[0].length || board[r][c].getText().length() > 0
			...
			open(r, c + 1);
			/*
			 * açılan kutucukların etrafını hesaplama boşları yazdırmıyor.
			 * Mayın olmayan kutuların tıklanan etrafını açma
			 */
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getButton() == 1) {
			...
			if (b.isMine()) {
				JOptionPane.showMessageDialog(frame, "You stepped on mine,Game Over!");
				print();
			} else {
